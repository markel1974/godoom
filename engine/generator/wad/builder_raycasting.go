...
func (b *Builder1) scanSubSectors() []*model.ConfigSector {
	...
	for subSectorId := uint16(0); int(subSectorId) < len(b.level.SubSectors); subSectorId++ {
		...
		for segmentId := subSector.StartSeg; segmentId < subSector.StartSeg+subSector.NumSegments; segmentId++ {
			...
			if lower != "-" {
				b.textures[lower] = true
			}
			if middle != "-" {
				b.textures[middle] = true
			}
			if upper != "-" {
				b.textures[upper] = true
			}
		...
		for _, s := range miSector.Segments {
			...
			*/
			...
			case 0:
				segments = append(segments, s)
			case 1:
				segments = append(segments, res[0])
			default:
...
func (b *Builder1) compileConvexHull(miSectors []*model.ConfigSector) {
	...
	for _, miSector := range miSectors {
		...
		var chs []*polygons.CHSegment
		for _, s := range miSector.Segments {
			c := polygons.NewCHSegment(miSector.Id, s, s.Start, s.End)
			chs = append(chs, c)
		}
...
func (b *Builder1) compileSegmentRelations(miSectors []*model.ConfigSector) {
	...
	for _, xSector := range miSectors {
		for _, xSegment := range xSector.Segments {
			//TODO REMOVE
			if xSegment.Kind == model.DefinitionWall {
				continue
			}
...
func (b *Builder1) segmentOnSegment(refSeg *model.ConfigSegment, currSeg *model.ConfigSegment) (model.XY, model.XY, bool) {
	...
	if _, sa0, _ := geometry.PointLine(refStart, currStart, currEnd); sa0.Has(geometry.OnPoint0Segment) || sa0.Has(geometry.OnPoint1Segment) {
		p1 = append(p1, model.XY{X: refStart.X, Y: refStart.Y})
	}
	if _, sb0, _ := geometry.PointLine(refEnd, currStart, currEnd); sb0.Has(geometry.OnPoint0Segment) || sb0.Has(geometry.OnPoint1Segment) {
	...
	var p2 []model.XY
	if _, sc0, _ := geometry.PointLine(currStart, refStart, refEnd); sc0.Has(geometry.OnPoint0Segment) || sc0.Has(geometry.OnPoint1Segment) {
		p2 = append(p2, model.XY{X: currStart.X, Y: currStart.Y})
	}
	if _, sd0, _ := geometry.PointLine(currEnd, refStart, refEnd); sd0.Has(geometry.OnPoint0Segment) || sd0.Has(geometry.OnPoint1Segment) {
