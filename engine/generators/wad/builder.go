...
func (b *Builder) scanSubSectors(level *Level, bsp *BSP) []*model.ConfigSector {
	...
	for i := 0; i < numSS; i++ {
		...
		//TROVARE il SideDefs id per questo segmento
		//sideDef := level.SideDefs[0]
		//_, sideDef := bsp.level.SegmentSideDef(seg, lineDef)
		//TODO REAL TEXTURE
		//fmt.Println("--------------------")
		//fmt.Println(ds.FloorPic)
		//fmt.Println(ds.CeilingPic)
		//image, err := b.w.GetTextureImage(sideDef.MiddleTexture)
		//if err == nil {
		//	fmt.Println(image)
		//}
