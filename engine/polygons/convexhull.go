...
func NewCHSegment(ref string, data interface{}, start model.XY, end model.XY) *CHSegment {
	chs := &CHSegment{
		Ref:   ref,
		Data:  data,
		Start: start,
...
func (ch *ConvexHull) Create(id string, inputSegments []*CHSegment) []*CHSegment {
	...
	var lower model.XY
	if ch.isLower(headSeg.Start, headSeg.End) {
		lower = headSeg.Start
	} else {
		lower = headSeg.End
	}
...
func (ch *ConvexHull) getConnect(curr *CHSegment, segments []*CHSegment) int {
	for idx, target := range segments {
		if curr.End.X == target.Start.X && curr.End.Y == target.Start.Y {
			return idx
		}
	}
	return -1
...
func (ch *ConvexHull) findLowest(segments []*CHSegment) int {
	...
	for i := 1; i < len(segments); i++ {
		if ch.isLower(segments[i].Start, segments[lowest].Start) {
			lowest = i
		}
		if ch.isLower(segments[i].End, segments[lowest].End) {
			lowest = i
		}
	}
	return lowest
...
func (ch *ConvexHull) isLower(a model.XY, b model.XY) bool {
...
func (ch *ConvexHull) area2(a model.XY, b model.XY, c model.XY) float64 {
	area := (b.X-a.X)*(c.Y-a.Y) - (c.X-a.X)*(b.Y-a.Y)
	return area
}

func (ch *ConvexHull) isLeft(p0 model.XY, p1 model.XY, p2 model.XY) bool {
...
func (ch *ConvexHull) sameLine(a model.XY, b model.XY, c model.XY) bool {
	//dot product
	d := (b.X-a.X)*(c.Y-a.Y) + (c.X-a.X)*(b.Y-a.Y)
	return d <= 0
}

func (ch *ConvexHull) FromSector(sector *model.Sector) []*model.Segment {
	var chs []*CHSegment
