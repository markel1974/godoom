...
type MipTexture struct {
	...
	Width  uint32
	Height uint32
	Pixels [MipMapLevels][]byte // Array di 4 byte-slices (livello 0, 1, 2, 3)
}

// NewMipTextures legge il lump TEXTURES e decodifica tutte le texture contenute.
func NewMipTextures(rs io.ReadSeeker, lumpInfo *LumpInfo) ([]*MipTexture, error) {
	...
	for i, offset := range offsets {
		...
		name := FromNullTerminatingString(nameBytes)
