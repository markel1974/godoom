...
func (r *Portal) Grow() {
	...
	if oldSize == 0 {
		...
		newSize = oldSize * 2
		newData := make([]*model.CompiledVolume, newSize)
	...
	for _, face := range volume.GetFaces() {
		...
		if neighbor != nil {
			...
			if sectorCeilY > neighborCeilY {
				upperP := cs.Acquire(neighbor, model.IdUpper, ceilT, floorT, face.GetMaterialUpper(), wx1, wx2, wx1, wx2, wz1, wz2, u0, u1)
			...
			if sectorFloorY < neighborFloorY {
				lowerP := cs.Acquire(neighbor, model.IdLower, ceilT, floorT, face.GetMaterialLower(), wx1, wx2, wx1, wx2, wz1, wz2, u0, u1)
			...
			wallP := cs.Acquire(nil, model.IdWall, ceilT, floorT, face.GetMaterialMiddle(), wx1, wx2, wx1, wx2, wz1, wz2, u0, u1)
...
func (r *Portal) Traverse(fbw, fbh int32, vi *model.ViewMatrix) ([]*model.CompiledVolume, int) {
...
func (r *Portal) compileProjection(fbw, fbh int32, vi *model.ViewMatrix, volume *model.Volume, qi *QueueItem) ([]QueueItem, int) {
	...
	var cs *model.CompiledVolume = nil
	first := false
	outIdx := 0
	...
	for _, face := range volume.GetFaces() {
		...
		sectorYCeil := vi.ZDistance(volume.GetCeilY())
		sectorYFloor := vi.ZDistance(volume.GetFloorY())
		...
		if neighbor != nil {
			...
			middleT := face.GetMaterialMiddle()
