...
func (qi *QueueItem) Hash64() uint64 {
	return (uint64(qi.volume.GetModelId()) << 48) |
...
func (qi *QueueItem) Update(volume *model.Volume, x1 float64, x2 float64, y1t float64, y2t float64, y1b float64, y2b float64) {
	qi.volume = volume
	qi.x1 = x1
	qi.x2 = x2
	qi.y1t = y1t
