...
func (w *FrameVertices) CheckDuplicatedTriangles() {
	const floatsPerTri = vertexAlignment * 3
	...
	for i := 0; i+floatsPerTri <= w.len; i += floatsPerTri {
		...
		x2, y2, z2 := w.vertices[i+vertexAlignment], w.vertices[i+vertexAlignment+1], w.vertices[i+vertexAlignment+2]
		x3, y3, z3 := w.vertices[i+vertexAlignment*2], w.vertices[i+vertexAlignment*2+1], w.vertices[i+vertexAlignment*2+2]
