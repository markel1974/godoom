...
func (bt *batchTriangles) Slice(i, j int) ITriangles {
...
func (bt *batchTriangles) Update(t ITriangles) {
	bt.tri.Update(t)
}

func (bt *batchTriangles) Copy() ITriangles {
	return &batchTriangles{
		tri: bt.tri.Copy(),
...
func (bp *batchPicture) Draw(t ITargetTriangles) {
	bt := t.(*batchTriangles)
	if bp.dst != bt.dst {
		panic(fmt.Errorf("(%T).Draw: ITargetTriangles generated by different Batch", bp))
	}
	bt.draw(bp)
}
